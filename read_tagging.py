...
## opening class list 
Df_classes = pd.read_csv('tables/TaggingCategory.csv')

dict_allcats = {}
fewlabels = []
for curcat in Df_classes.columns:
    curcatlower = curcat.lower()
    list_curcat = []
    ...
    dict_allcats[curcatlower] = list_curcat
...
# Load label
with open('tables/class_labels_indices.csv', 'r') as f:
...
def read_tagging_file(pklfile):
    Df = load_obj_tag(pklfile)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Tagging file reader')
    parser.add_argument('--input', default=None, type=str, help='Path to pkl file')
    parser.add_argument('--save_path', default='.', type=str, help='Path to save output csv and pkl')
    ...
    Df_new = read_tagging_file(args.input)
